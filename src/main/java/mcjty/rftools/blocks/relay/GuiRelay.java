...
public class GuiRelay extends GenericGuiContainer<RelayTileEntity, GenericContainer> {
    ...
    public GuiRelay(RelayTileEntity relayTileEntity, GenericContainer container, PlayerInventory inventory) {
        super(RFTools.instance, RFToolsMessages.INSTANCE, relayTileEntity, container, inventory, GuiProxy.GUI_MANUAL_MAIN, "prelay");
    ...
    private Panel createRedstonePanel() {
        ImageLabel redstoneOff = new ImageLabel(minecraft, this).setImage(iconGuiElements, 16, 0);
        redstoneOff.setDesiredWidth(16).setDesiredHeight(16).setTooltips("Redstone signal off").setLayoutHint(70, 0, 16, 16);
        ImageLabel redstoneOn = new ImageLabel(minecraft, this).setImage(iconGuiElements, 32, 0);
    ...
    private Panel createSidePanel(int side) {
        ...
        Label label = new Label(minecraft, this).setText(labelText).setTooltips(tooltips[side]);
        label.setDesiredWidth(14).setDesiredHeight(14);
        return new Panel(minecraft, this).setLayout(new HorizontalLayout().setHorizontalMargin(1)).
                addChild(label).
    ...
    private Panel createSubPanel(int side, String redstoneState) {
        ...
        ImageChoiceLabel inputOutput = new ImageChoiceLabel(minecraft, this).
        ...
        TextField energyField = new TextField(minecraft, this).setTooltips("Amount of RF to input/output", "when redstone is " + redstoneState).
        ...
        Panel panel = new Panel(minecraft, this).setLayout(new HorizontalLayout().setHorizontalMargin(1)).
    ...
    private Button createEnergyOffsetButton(final TextField energyField, String label, final int amount) {
        return new Button(minecraft, this).setText(label).setDesiredHeight(14).setDesiredWidth(16).addButtonEvent(parent -> adjustEnergy(energyField, amount));
    }
