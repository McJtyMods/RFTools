...
//    public static final int SLOT_PLAYERHOTBAR = SLOT_PLAYERINV + 3*9;
    ...
    private void layoutBuffer() {
        ...
        int leftCol = 12;
        int topRow = 97;
        addSlotBox(new SlotDefinition(SlotType.SLOT_INPUT), CONTAINER_INVENTORY, SLOT_BUFFER, leftCol, topRow, 13, 18, 2, 18);

        // Output slots
        leftCol = 31;
        topRow = 142;
        addSlotBox(new SlotDefinition(SlotType.SLOT_OUTPUT), CONTAINER_INVENTORY, SLOT_BUFFEROUT, leftCol, topRow, 2, 18, 2, 18);
    }

    private void layoutCraftingGrid() {
        int leftCol = 193;
        int topRow = 7;
        addSlotBox(new SlotDefinition(SlotType.SLOT_GHOST), CONTAINER_INVENTORY, SLOT_CRAFTINPUT, leftCol, topRow, 3, 18, 3, 18);
        addSlot(new SlotDefinition(SlotType.SLOT_GHOSTOUT), CONTAINER_INVENTORY, SLOT_CRAFTOUTPUT, leftCol, 65);
    }
